...
const ProductsTable = ({ products, productData }) => {
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: 'Buying Price',
        size: 150,
        cell: ({ row }) => `Ksh.${row.original.buying_price}`,
      },
      {
        accessorKey: 'selling_price',
        header: 'Selling Price',
        size: 150,
        cell: ({ row }) => `Ksh.${row.original.selling_price}`,
      },
      {
        id: 'actions',
        header: 'Actions',
  ...
  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(productData.map(item => ({
      ID: item.id,
      Name: item.name,
      'Buying Price (Ksh)': item.buying_price,
